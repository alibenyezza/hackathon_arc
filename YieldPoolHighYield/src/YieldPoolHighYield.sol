...
/**
 * @title YieldPoolHighYield
 * @author TreasuryPilot Team
 * @notice A simulated high-risk/high-yield pool for the TreasuryPilot system on Arc.
 * This pool represents an aggressive DeFi lending strategy with higher returns.
 * 
 * @dev This contract simulates a 9% APY yield pool. The SafeVault (treasury) can
 ...
 */
contract YieldPoolHighYield is Ownable, ReentrancyGuard {
    /// @notice The USDC token contract on Arc network
    IERC20 public immutable usdcToken;

    /// @notice Annual Percentage Yield in basis points (900 = 9.00%)
    ...
    uint8 public constant RISK_TIER = 3;
     ...
     * @param _initialAPYBps The initial APY in basis points (e.g., 900 for 9%)
     */
    constructor(
        ...
        require(_usdcAddress != address(0), "YieldPoolHighYield: USDC address cannot be zero");
        require(_initialAPYBps > 0, "YieldPoolHighYield: APY must be greater than zero");
    ...
    function deposit(uint256 _amount) external nonReentrant {
        require(_amount > 0, "YieldPoolHighYield: Deposit amount must be greater than zero");
        
        uint256 beforeBalance = usdcToken.balanceOf(address(this));
        usdcToken.transferFrom(msg.sender, address(this), _amount);
        uint256 afterBalance = usdcToken.balanceOf(address(this));

        require(afterBalance == beforeBalance + _amount, "YieldPoolHighYield: Deposit transfer failed");
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        require(_amount > 0, "YieldPoolHighYield: Withdraw amount must be greater than zero");
        require(deposits[msg.sender] >= _amount, "YieldPoolHighYield: Insufficient deposit balance");
        require(totalAssets >= _amount, "YieldPoolHighYield: Insufficient pool liquidity");

        deposits[msg.sender] -= _amount;
        totalAssets -= _amount;

        bool success = usdcToken.transfer(msg.sender, _amount);
        require(success, "YieldPoolHighYield: Withdrawal transfer failed");
    ...
    function accumulateYield() external {
        ...
        // APY is in basis points (900 = 9%), so we divide by 10000
    ...
    function setAPY(uint256 _newAPYBps) external onlyOwner {
        require(_newAPYBps > 0, "YieldPoolHighYield: APY must be greater than zero");
     ...
     * @return _riskTier Risk tier (3 = High)
     */
