...
/**
 * @title YieldPoolModerate
 * @author TreasuryPilot Team
 * @notice A simulated medium-risk yield pool for the TreasuryPilot system on Arc.
 * This pool represents a balanced DeFi lending strategy with moderate returns.
 * 
 * @dev This contract simulates a 5% APY yield pool. The SafeVault (treasury) can
 ...
 */
contract YieldPoolModerate is Ownable, ReentrancyGuard {
    ...
    uint8 public constant RISK_TIER = 2;
     ...
     * @param _initialAPYBps The initial APY in basis points (e.g., 500 for 5%)
     */
    constructor(
        ...
        require(_usdcAddress != address(0), "YieldPoolModerate: USDC address cannot be zero");
        require(_initialAPYBps > 0, "YieldPoolModerate: APY must be greater than zero");
    ...
    function deposit(uint256 _amount) external nonReentrant {
        require(_amount > 0, "YieldPoolModerate: Deposit amount must be greater than zero");
        
        uint256 beforeBalance = usdcToken.balanceOf(address(this));
        usdcToken.transferFrom(msg.sender, address(this), _amount);
        uint256 afterBalance = usdcToken.balanceOf(address(this));

        require(afterBalance == beforeBalance + _amount, "YieldPoolModerate: Deposit transfer failed");
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        require(_amount > 0, "YieldPoolModerate: Withdraw amount must be greater than zero");
        require(deposits[msg.sender] >= _amount, "YieldPoolModerate: Insufficient deposit balance");
        require(totalAssets >= _amount, "YieldPoolModerate: Insufficient pool liquidity");

        deposits[msg.sender] -= _amount;
        totalAssets -= _amount;

        bool success = usdcToken.transfer(msg.sender, _amount);
        require(success, "YieldPoolModerate: Withdrawal transfer failed");
    ...
    function accumulateYield() external {
        ...
        // APY is in basis points (500 = 5%), so we divide by 10000
    ...
    function setAPY(uint256 _newAPYBps) external onlyOwner {
        require(_newAPYBps > 0, "YieldPoolModerate: APY must be greater than zero");
     ...
     * @return _riskTier Risk tier (2 = Medium)
     */
